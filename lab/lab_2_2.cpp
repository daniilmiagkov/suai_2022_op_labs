...
void lab_2_2()
{
    ...
    vector <string> As;
    in_vector(As);//ввод исходного массива строк

    string Is = input_Is();
    As.push_back(Is);//ввод строки
    
    vector <vector <int>> Hs(As.size(), vector <int> (2));

    fill_Hs(Hs, As);//заполнение массива хэшей

    int hash_Is = Hs[Hs.size() - 1][0];//хэш введеной строк в конце массива хэшей
